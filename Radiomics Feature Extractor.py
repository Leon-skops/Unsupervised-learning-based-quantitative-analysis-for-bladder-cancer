...
def main():
  outPath = r'F:\Radiomics_project'

  inputCSV = os.path.join(outPath, 'Input_data.csv')
  outputFilepath = os.path.join(outPath, 'Features.csv')
  progress_filename = os.path.join(outPath, 'Records.txt')
  params = os.path.join(outPath,  'CT_parameters.yaml')
  ...
  for entry in flists:  # Loop over all columns (i.e. the test cases)
    ...
    if (imageFilepath is not None) and (maskFilepath is not None):
      ...
      try:
        ...
        featureVector = featureVector._append(result)
      except Exception:
